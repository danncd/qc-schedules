...
import { colorGPA, colorPassingRate, colorWithdrawalRate, getCourseStats, getGradeData } from "@/utils/client/utils";
...
export const CourseCard = memo(function CourseCard({ course }: Props) {
    ...
    const { gpa, withdrawalRate, passingRate } = getCourseStats(course);
    return (
        <div className="rounded-lg border border-gray-300 shadow-md dark:shadow-none p-4 flex flex-col gap-4">
            ...
            <div className="p-4 bg-gray-200/60 dark:bg-[#212121] rounded-lg">
                ...
                <Description description="Average GPA" value={gpa} color={colorGPA(gpa)}/>
                <Description description="Passing Rate (Above C)" value={`${passingRate}%`} color={colorPassingRate(passingRate)}/>
                <Description description="Withdrawal Rate" value={`${withdrawalRate}%`} color={colorWithdrawalRate(withdrawalRate)}/>
...
function Description({ description, value, color }: DescriptionProps) {
    return (
        <div className="text-sm">
            <span>{description}</span> : <span className={`${manrope.className} font-extrabold ${color}`}>{value}</span>
        </div>
    )
}

const GradeDistributionChart = memo(({ gradeData }: { gradeData: GradeRecord[] }) => {
